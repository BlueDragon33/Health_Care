...
function seededState(): HealthLocalState {
  ...
  for (let offset = -6; offset <= 0; offset += 1) {
    ...
    day.meals = [
      { id: `meal-${offset}`, meal: "breakfast", text: "Phở bò, sữa chua, chuối", createdAt: new Date().toISOString() },
    ];
    day.activities = [
      { id: `act-${offset}`, type: offset % 2 === 0 ? "Chạy" : "Đi bộ", minutes: 25 + (offset + 6) * 3, createdAt: new Date().toISOString() },
    ];
...
export default function VisualPreviewClient() {
  ...
  useEffect(() => {
    ...
    const secondaryId = "preview-minh-khang";
    const registry: HealthProfileRegistry = {
      ...
      profiles: [
        ...
        { id: secondaryId, displayName: "Minh Khang", birthDate: "2016-05-12", sexForGrowthReference: "male", createdAt: now, updatedAt: now },
      ],
    };
    saveHealthProfileRegistry(registry);
    saveHealthProfileState(primaryId, seededState());
    const second = createInitialHealthState();
    second.profile = { name: "Minh Khang", birthDate: "2016-05-12", sex: "male", note: "" };
