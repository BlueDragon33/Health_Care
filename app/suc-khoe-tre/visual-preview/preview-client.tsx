...
function seededState(): HealthLocalState {
  ...
  state.profile = { name: "Bảo Anh", birthDate: "2013-07-25", sex: "female", note: "Preview UI V5" };
  state.growth = [
  ...
  for (let offset = -6; offset <= 0; offset += 1) {
    ...
    day.feeling = "good";
    if (offset === 0) day.journalNote = "Hôm nay khỏe, học tập và vận động bình thường.";
    state.days[key] = day;
  }
  return state;
}
...
export default function VisualPreviewClient() {
  ...
  useEffect(() => {
    ...
    const infantId = "preview-be-minh";
    const registry: HealthProfileRegistry = {
      ...
      profiles: [
        ...
        { id: infantId, displayName: "Bé Minh", birthDate: "2025-12-09", sexForGrowthReference: "male", createdAt: now, updatedAt: now },
      ],
    };
    saveHealthProfileRegistry(registry);
    saveHealthProfileState(primaryId, seededState());
    const infant = createInitialHealthState();
    infant.profile = { name: "Bé Minh", birthDate: "2025-12-09", sex: "male", note: "Hồ sơ thử phạm vi 9 tháng" };
    infant.growth = [{ id: "infant-g-1", date: todayKey(), heightCm: 71.2, weightKg: 8.6 }];
    saveHealthProfileState(infantId, infant);
    setReady(true);
  }, []);

  if (!ready) return <div style={{ padding: 40 }}>Đang dựng preview V5…</div>;
