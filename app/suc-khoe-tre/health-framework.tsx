      ...
      <section className="hf-content">
        ...
        {active === "growth" ? <section className="hf-section">
          ...
          <div className="hf-work-grid">
            ...
            <section className="hf-panel"><span className="hf-kicker">Mốc gần nhất</span><div className="hf-metric-row"><div><strong>{latestGrowth?.heightCm ?? "—"}</strong><small>cm</small></div><div><strong>{latestGrowth?.weightKg ?? "—"}</strong><small>kg</small></div><div><strong>{growthSummary.value}</strong><small>{growthSummary.label}</small></div></div><p className="hf-muted">{growthSummary.detail}</p>{latestAssessment?.available ? <><div className="hf-local-badge">{formatAgeMonths(latestAssessment.ageMonths)} · z {latestAssessment.zScore >= 0 ? "+" : ""}{latestAssessment.zScore.toFixed(2)} · {latestAssessment.categoryLabel}</div><p className="hf-muted">Ngưỡng tại tháng tuổi này: gầy &lt; {latestAssessment.cutoffs.thinnessBelow.toFixed(1)} · thừa cân &gt; {latestAssessment.cutoffs.overweightAbove.toFixed(1)} · béo phì &gt; {latestAssessment.cutoffs.obesityAbove.toFixed(1)} kg/m².</p></> : <div className="hf-age-warning">{latestAssessment?.message ?? "Chưa có số đo để đánh giá."}</div>}<p className="hf-muted">{profileAge.months !== null && profileAge.months < 60 ? "Nguồn chuẩn cho trẻ dưới 5 tuổi: WHO Child Growth Standards 0–5. Không dùng bảng BMI-for-age 5–19 để phân loại nhóm này." : "Nguồn: WHO Reference 2007 · BMI-for-age 5–19 years. Ứng dụng chủ động giới hạn phạm vi sản phẩm đến hết 18 tuổi 11 tháng."}</p></section>
          </div>
