...
export default function HealthFramework({ initialCourse, device }: { initialCourse: unknown; device: HealthDeviceAccess }) {
  ...
  const growthStage = useMemo(() => growthConfigForLifeStage(profileAge.lifeStage), [profileAge.lifeStage]);
      ...
      <section className="hf-content">
        ...
        {active === "growth" ? <section className="hf-section">
          ...
          <div className="hf-work-grid">
            <section className="hf-panel"><div className="hf-panel-head"><div><span className="hf-kicker">Ghi số đo</span><h3>Thêm mốc tăng trưởng</h3></div></div><div className="hf-form-grid"><label>Ngày đo<input type="date" max={today} value={growthDate} onChange={(event) => setGrowthDate(event.target.value)} /></label><label>{growthStage.statureLabel}<input inputMode="decimal" value={heightCm} onChange={(event) => setHeightCm(event.target.value)} placeholder={growthStage.staturePlaceholder} /></label><label>Cân nặng (kg)<input inputMode="decimal" value={weightKg} onChange={(event) => setWeightKg(event.target.value)} placeholder={growthStage.weightPlaceholder} /></label></div><p className="hf-muted">{growthStage.measurementHint}</p><button className="hf-primary" type="button" onClick={addGrowth}>Lưu số đo trên thiết bị</button></section>
            <section className="hf-panel"><span className="hf-kicker">Mốc gần nhất</span><div className="hf-metric-row"><div><strong>{latestGrowth?.heightCm ?? "—"}</strong><small>cm</small></div><div><strong>{latestGrowth?.weightKg ?? "—"}</strong><small>kg</small></div><div><strong>{growthSummary.value}</strong><small>{growthSummary.label}</small></div></div><p className="hf-muted">{growthSummary.detail}</p>{profileAge.months !== null && profileAge.months < 108 ? null : latestAssessment?.available ? <><div className="hf-local-badge">{formatAgeMonths(latestAssessment.ageMonths)} · z {latestAssessment.zScore >= 0 ? "+" : ""}{latestAssessment.zScore.toFixed(2)} · {latestAssessment.categoryLabel}</div><p className="hf-muted">Ngưỡng tại tháng tuổi này: gầy &lt; {latestAssessment.cutoffs.thinnessBelow.toFixed(1)} · thừa cân &gt; {latestAssessment.cutoffs.overweightAbove.toFixed(1)} · béo phì &gt; {latestAssessment.cutoffs.obesityAbove.toFixed(1)} kg/m².</p></> : <div className="hf-age-warning">{latestAssessment?.message ?? "Chưa có số đo để đánh giá."}</div>}<p className="hf-muted">{profileAge.months !== null && profileAge.months < 60 ? "Nguồn chuẩn cho trẻ dưới 5 tuổi: WHO Child Growth Standards 0–5. Không dùng bảng BMI-for-age 5–19 để phân loại nhóm này." : profileAge.months !== null && profileAge.months < 108 ? "Nguồn tham chiếu nền: WHO Reference 2007 · 5–19 years. Bảng LMS định lượng 5–8 tuổi chưa được tích hợp/kiểm định trong ứng dụng nên chỉ hiển thị số đo thô và xu hướng." : "Nguồn: WHO Reference 2007 · BMI-for-age. Bảng LMS đã tích hợp của ứng dụng hiện dùng từ 9 tuổi 0 tháng đến hết 18 tuổi 11 tháng."}</p></section>
          </div>
          ...
          <section className="hf-panel hf-list-panel"><div className="hf-panel-head"><div><span className="hf-kicker">Timeline</span><h3>Lịch sử số đo</h3></div><small>{growth.length} mốc</small></div>{growth.length ? <div className="hf-table-list">{growth.map((entry) => { const assessment = assessmentFor(entry); const presentation = timelinePresentationFor(entry); return <article key={entry.id}><strong>{formatDate(entry.date)}</strong><span>{entry.heightCm} cm · {entry.weightKg} kg</span><span>{presentation.metricLabel}</span><span>{presentation.useWhoBmiReference ? (assessment.available ? `${formatAgeMonths(assessment.ageMonths)} · z ${assessment.zScore >= 0 ? "+" : ""}${assessment.zScore.toFixed(2)} · ${assessment.categoryLabel}` : assessment.message) : presentation.detail}</span><button type="button" onClick={() => setState((current) => ({ ...current, growth: current.growth.filter((item) => item.id !== entry.id) }))}>Xóa</button></article>; })}</div> : <Empty>Chưa có số đo tăng trưởng.</Empty>}</section>
        </section> : null}
