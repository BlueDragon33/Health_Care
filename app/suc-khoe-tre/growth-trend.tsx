...
import type { GrowthEntry, HealthProfile } from "./health-local-store";
import { assessWhoBmiForAge, calculateBmi, formatAgeMonths } from "./who-bmi-reference";
...
const METRIC_OPTIONS: { id: GrowthTrendMetric; label: string }[] = [
  { id: "height", label: "Chiều cao" },
  { id: "weight", label: "Cân nặng" },
  { id: "bmi-z", label: "BMI-for-age WHO" },
];
...
function metricTitle(metric: GrowthTrendMetric) {
  if (metric === "height") return "Xu hướng chiều cao ghi nhận";
...
export default function GrowthTrend({ entries, profile }: { entries: GrowthEntry[]; profile: HealthProfile }) {
  ...
  const points = useMemo(() => metricPoints(filtered, metric, profile), [filtered, metric, profile]);
  const domain = useMemo(() => yDomain(points, metric), [points, metric]);
  ...
  const thresholdLines = metric === "bmi-z"
    ? [
      { value: -3, label: "−3 SD" },
      { value: -2, label: "−2 SD" },
      { value: 1, label: "+1 SD" },
  ...
  const unavailableBmi = metric === "bmi-z" && filtered.length > 0 && points.length === 0;
  ...
  return <section className="growth-trend-card" aria-labelledby="growth-trend-title">
    <header className="growth-trend-head">
      <div>
        ...
        <h3 id="growth-trend-title">{metricTitle(metric)}</h3>
      ...
      <div className="growth-trend-summary">
        ...
        {delta !== null ? <small>Thay đổi từ mốc đầu: {delta >= 0 ? "+" : ""}{delta.toFixed(metric === "bmi-z" ? 2 : 1)} {unitFor(metric)}</small> : null}
      </div>
    </header>
    ...
    <div className="growth-trend-controls" aria-label="Tùy chọn biểu đồ tăng trưởng">
      <div className="growth-trend-buttons" aria-label="Chỉ số">
        {METRIC_OPTIONS.map((option) => <button
          type="button"
          key={option.id}
          className={metric === option.id ? "growth-3d-button is-selected" : "growth-3d-button"}
    ...
    {unavailableBmi ? <div className="growth-trend-warning">
      ...
      <span>Cần ngày sinh và giới tính trong Hồ sơ, đồng thời mốc đo phải nằm trong phạm vi 9 tuổi 0 tháng đến hết 18 tuổi 11 tháng. Chiều cao và cân nặng thô vẫn xem được.</span>
    </div> : null}

    {points.length ? <>
      <div className="growth-chart-scroll">
        <svg className="growth-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${metricTitle(metric)}, ${points.length} mốc từ ${formatDate(points[0].date)} đến ${formatDate(points.at(-1)!.date)}`}>
          {yTicks.map((tick) => <g key={tick}>
            <line className="growth-grid-line" x1={left} x2={width - right} y1={y(tick)} y2={y(tick)} />
            <text className="growth-axis-label" x={left - 10} y={y(tick) + 4} textAnchor="end">{tick.toFixed(metric === "bmi-z" ? 1 : 0)}</text>
          </g>)}
          ...
          <text className="growth-axis-title" x={16} y={top + chartHeight / 2} transform={`rotate(-90 16 ${top + chartHeight / 2})`} textAnchor="middle">{unitFor(metric)}</text>
        </svg>
      </div>
      {metric === "bmi-z" ? <div className="growth-threshold-note"><strong>Đường tham chiếu WHO:</strong><span>−3 SD gầy nghiêm trọng · −2 SD gầy · +1 SD thừa cân · +2 SD béo phì. Đây là ngưỡng tham chiếu tăng trưởng, không phải chẩn đoán bệnh.</span></div> : <div className="growth-threshold-note"><strong>Số đo thô:</strong><span>Biểu đồ này chỉ cho thấy thay đổi chiều cao/cân nặng theo các lần đo. Không gắn percentile hay phân loại lâm sàng khi chưa có bộ tham chiếu tương ứng đã được kiểm chứng.</span></div>}
