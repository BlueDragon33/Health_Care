...
const origin=(name,optional=false)=>{const raw=String(process.env[name]??"").trim(); if(!raw&&optional) return ""; if(!raw) throw new Error(`${name} chưa được cấu hình.`); const u=new URL(raw); if(u.protocol!=="https:"||u.username||u.password||u.pathname!=="/"||u.search||u.hash) throw new Error(`${name} phải là HTTPS origin thuần.`); if(u.hostname.endsWith(".chatgpt.site")) throw new Error(`${name} không được dùng ChatGPT Sites.`); return u.origin;};
...
const rendered=fs.readFileSync(TEMPLATE,"utf8")
 ...
 .replace("__APPLICATION_MANAGEMENT_PRODUCTION_ORIGIN__",origin("APPLICATION_MANAGEMENT_PRODUCTION_ORIGIN",true))
 .replace("__HEALTH_BUILD_REVISION__",revision)
 .replace("__HEALTH_ACCESS_MODE__",String(process.env.HEALTH_ACCESS_MODE||"standalone").trim().toLowerCase()==="standalone"?"standalone":"managed");
